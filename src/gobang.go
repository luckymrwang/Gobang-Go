...
func main() {
	...
	e.Renderer = t
	e.GET("/status", echo.WrapHandler(gobang.HandleStatusSocket()))
	e.GET("/socket", echo.WrapHandler(gobang.HandleGameSocket()))
	e.GET("/game", gobang.Game)
	e.Any("/login", Login)
	if err := e.Start(":" + strconv.Itoa(config.port)); err != nil {
		panic(err)
	}
}
